 ...
 * @param {Connection} [optns.connection = mongoose.connection] A valid instance of mongoose connection.
 * @param {String} [optns.bucketName="fs"] The 'files' and 'chunks' collections will be prefixed with the bucket name followed by a dot.
 * @param {Number} [optns.chunkSizeBytes=255 * 1024] Number of bytes stored in each chunk. Defaults to 255KB
 * @param {Object} [optns.writeConcern] Optional write concern to be passed to write operations, for instance `{ w: 1 }`
 * @param {Object} [optns.readPreference] Optional read preference to be passed to read operations
...
//TODO ensure mongoose default is connected
function createBucket(optns = {}) {
  ...
  if (!isConnected(connection)) {
    const error = new Error('Mongoose is not connected');
    error.status = 500;
    throw error;
  }
...
function createModel(optns = {}) {
  ...
  options.connection = (options.connection || mongoose.connection);

  // create bucket
  // const bucketName = bucketName || toCollectionName(modelName);
  const bucket = createBucket(options);

  // return model
  // const schema = createSchema({ bucket, collectionName })
  // const _model = model(modelName, schema, connection);
  return bucket;
}
