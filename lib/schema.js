...
module.exports = function (gridFSStorage) {
  ...
  const mongooseConnection = gridFSStorage.options.mongooseConnection
  ...
  GridFSSchema.methods.write = function (stream, done) {
    ...
    // grab gridfs
    const gridfs = mongooseConnection.model(model).gridfs
    ...
    gridfs.write(fileDetails, stream, function (error, createdFile) {
      if (error) {
        ...
        // read file details
        mongooseConnection.model(model).findById(createdFile._id, done)
      }
    })
  }
  ...
  GridFSSchema.methods.unlink = function (done) {
    ...
    // obtain file details
    mongooseConnection.model(model).findById(this._id, function (error, fileDetails) {
      if (error) {
        done(error)
      } else {
