...
module.exports = function(gridFSStorage) {
	...
	GridFSSchema.methods.write = function(stream, done) {
		...
		//grab gridfs
		var gridfs = mongoose.connections[gridFSStorage.options.connection].model(model).gridfs;
		...
		gridfs.write(fileDetails, stream, function(error, createdFile) {
			if (error) {
				...
				//read file details
				mongoose.connections[gridFSStorage.options.connection].model(model).findById(createdFile._id, done);
			}
		});
	};
	...
	GridFSSchema.methods.unlink = function(done) {
		...
		//obtain file details
		mongoose.connections[gridFSStorage.options.connection].model(model).findById(this._id, function(error, fileDetails) {
			if (error) {
				done(error);
			} else {
